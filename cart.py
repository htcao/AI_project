...
def main():
    ...
    feature_pd = net.features(data_pd).view(data_control.size(0), 256 * 6 * 6 * 2).detach().numpy()
    age_ctr = np.load('age.npy')
    age_pd = np.load('PDage.npy')
    feature_ctr = np.concatenate((feature_ctr, age_ctr[:, 0].reshape(-1, 1)), axis=1)
    feature_pd = np.concatenate((feature_pd, age_pd[:, 0].reshape(-1, 1)), axis=1)
    y_control = -np.ones(feature_ctr.shape)
    y_pd = np.ones(feature_pd.shape)
